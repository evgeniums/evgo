...
type Subscriber interface {
	...
	Handle(sctx context.Context, topicName string, msg []byte) error
}
...
func (s *SubscriberBase) Handle(sctx context.Context, topicName string, msg []byte) error {
	s.mutex.RLock()
	topics, ok := s.topics[topicName]
	s.mutex.RUnlock()
	if !ok {
	...
	for _, topic := range topics {
		err := topic.Handle(sctx, msg, s.serializer)
		if err != nil {
			return err
		}
	}
...
func (s *SubscriberBase) NewOpContext(topicName string) (op_context.Context, context.Context) {
