...
import (
	...
	"github.com/evgeniums/evgo/pkg/admin/admin_api/admin_api_client"
...
func initTest(t *testing.T) *TestContext {
	...
	ctx.TargetUser, err = ctx.LocalAdminManager.Add(ctx.AdminOpCtx, targetAdminLogin, targetAdminPassword, user.Phone(targetAdminPhone, &Admin{}), user.Email(targetAdminEmail, &Admin{}))
...
func TestAdd(t *testing.T) {
	...
	phone1 := "999000111"
	addedAdmin1, err := ctx.RemoteAdminManager.AddAdmin(ctx.ClientOpCtx, login1, password1, phone1)
	require.NoError(t, err)
	require.NotNil(t, addedAdmin1)
	assert.Equal(t, login1, addedAdmin1.Login())
	assert.Equal(t, phone1, addedAdmin1.Phone())

	dbAdmin1, err := ctx.LocalAdminManager.FindByLogin(ctx.AdminOpCtx, login1)
	require.NoError(t, err)
	...
	addedAdmin2, err := ctx.RemoteAdminManager.Add(ctx.ClientOpCtx, login2, password2, user.Phone(phone2, &Admin{}), user.Email(email2, &Admin{}))
	require.NoError(t, err)
	...
	dbAdmin2, err := ctx.LocalAdminManager.FindByLogin(ctx.AdminOpCtx, login2)
	require.NoError(t, err)
...
func TestSetPhone(t *testing.T) {
	...
	// check unknown ID
	err := ctx.RemoteAdminManager.SetPhone(ctx.ClientOpCtx, targetAdminLogin, newPhone)
	test_utils.CheckGenericError(t, err, generic_error.ErrorCodeNotFound)
	ctx.Reset()

	dbAdmin1, err := ctx.LocalAdminManager.FindByLogin(ctx.AdminOpCtx, targetAdminLogin)
	require.NoError(t, err)
	require.NotNil(t, dbAdmin1)
	...
	// check invalid phone
	err = ctx.RemoteAdminManager.SetPhone(ctx.ClientOpCtx, ctx.TargetUser.GetID(), "not phone")
	...
	// check success
	assert.NoError(t, ctx.RemoteAdminManager.SetPhone(ctx.ClientOpCtx, ctx.TargetUser.GetID(), newPhone))
	dbAdmin2, err := ctx.LocalAdminManager.FindByLogin(ctx.AdminOpCtx, targetAdminLogin)
	require.NoError(t, err)
	require.NotNil(t, dbAdmin2)
...
func TestSetEmail(t *testing.T) {
	...
	// check invalid ID
	err := ctx.RemoteAdminManager.SetEmail(ctx.ClientOpCtx, targetAdminLogin, newEmail)
	test_utils.CheckGenericError(t, err, generic_error.ErrorCodeNotFound)
	ctx.Reset()
	...
	// check invalid email
	err = ctx.RemoteAdminManager.SetEmail(ctx.ClientOpCtx, ctx.TargetUser.GetID(), "not email")
	...
	// check success
	assert.NoError(t, ctx.RemoteAdminManager.SetEmail(ctx.ClientOpCtx, ctx.TargetUser.GetID(), newEmail))
	dbAdmin2, err := ctx.LocalAdminManager.FindByLogin(ctx.AdminOpCtx, targetAdminLogin)
	require.NoError(t, err)
	require.NotNil(t, dbAdmin2)
...
func TestFindUsers(t *testing.T) {
	...
	admins, count, err := ctx.RemoteAdminManager.FindUsers(ctx.ClientOpCtx, nil)
	require.NoError(t, err)
	...
	filter.Count = true
	admins, count, err = ctx.RemoteAdminManager.FindUsers(ctx.ClientOpCtx, filter)
	require.NoError(t, err)
...
func TestFindSingleUser(t *testing.T) {
	...
	admin, err := ctx.RemoteAdminManager.Find(ctx.ClientOpCtx, ctx.TargetUser.GetID())
	require.NoError(t, err)
...
func TestSetBlocked(t *testing.T) {
	...
	assert.NoError(t, ctx.RemoteAdminManager.SetBlocked(ctx.ClientOpCtx, ctx.TargetUser.Login(), true, true))
	dbAdmin, err := ctx.LocalAdminManager.FindByLogin(ctx.AdminOpCtx, targetAdminLogin)
	require.NoError(t, err)
	require.NotNil(t, dbAdmin)
	assert.True(t, dbAdmin.IsBlocked())

	assert.NoError(t, ctx.RemoteAdminManager.SetBlocked(ctx.ClientOpCtx, ctx.TargetUser.Login(), false, true))
	dbAdmin, err = ctx.LocalAdminManager.FindByLogin(ctx.AdminOpCtx, targetAdminLogin)
	require.NoError(t, err)
	require.NotNil(t, dbAdmin)
...
func TestSetPassword(t *testing.T) {
	...
	assert.NoError(t, ctx.RemoteAdminManager.SetPassword(ctx.ClientOpCtx, ctx.TargetUser.GetID(), newPassword))
